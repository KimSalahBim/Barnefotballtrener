...
(function () {
  ...
  async function deleteTeam(teamId) {
    ...
    try {
      ...
      // Slett workouts (treningsøkter/maler) for dette laget
      try {
        await sb.from('workouts').delete().eq('user_id', uid).eq('team_id', teamId);
      } catch (_) {}
      ...
      rows.forEach(function(row) {
        ...
        if (row.key === 'liga' && row.value) {
          try {
            ...
            if (window.liga) window.liga.render();
          } catch (_) {}
        }
      });
...
`;
  ...
  window.initApp = function initApp() {
    ...
    (async function() {
      ...
      setupPlayersUI();
      setupGroupingUI();
      if (window.liga) window.liga.init({
        state: state,
        saveState: saveState
      });
